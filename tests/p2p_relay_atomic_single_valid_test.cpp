...
int main() {
    ...
    const auto roundtrip =
        caesar::Block::deserialize_full(
            valid_block.serialize_full_binary());

    assert(roundtrip.hash() == valid_block.hash());
    std::cerr << "[ATOMIC] block roundtrip=OK" << std::endl;

    client.send_frame(
        make_blocks_frame({valid_block}));
